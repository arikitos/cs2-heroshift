# Boolean hook semantics (REFACTOR.md section 11)

Characterized directly from the legacy dispatch in `src/player/PlayerEvents.cs` and
`src/player/EntityEvents.cs` before `SkillDispatcher` replaced it. `SkillDispatcherTests`
...
- **Signature exists** (`bool WeaponDrop(DynamicHook, CCSPlayerController)`) and is
  **implemented by one skill** (`Iana`), but **no call site in the legacy codebase ever
  dispatches it** - it is dead/unreachable code today. This is a pre-existing oddity, not
  something to "fix": REFACTOR.md section 2 requires preserving questionable behavior as-is
  and documenting it here rather than silently making the hook reachable (which would be a
